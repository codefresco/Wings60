...
- `case-nice-nano-and-rp2040-usb-movable.3mf` is for builds that use MCU board with usb inset into the board, like nice nano for wireless or Tenstar RP2040 pro micro for wired. This case by default is set for 5mm total socket height and has opening for the power switch. By adjustging negative objects you can move the USB height up and down or remove the opening for the power switch for wired builds. This is the most adjustable 3mf file.
- `case-for-pcb-mount-switches.3mf` is for pcb mount switches and is more challenging than others. The top plate is optional, and does not hold the swithes because it is designed to come off even after switches are soldered on. Switches sit directly on the pcb with this build. If using the top plate, make sure it is on when soldering switches, switches are sitting well on top of the pcb, and the plate is also fully contacting the PCB. Because of the tight tolerance if you solder switches first without the top case it might not be possible to put it on afterwards. Easiest way is to tape it to the pcb, tape and solder switches in all corners, then solder the rest of the switches. 
