...
- `case-for-pcb-mount-switches.3mf` is for pcb mount switches. The top plate is optional, and does not hold the swithes because it is designed to come off even after switches are soldered on. Switches sit directly on the pcb with this build. If using the top plate, make sure it is on when soldering switches, because of the tight tolerance if you solder switches first without the top case it might not be possible to put it on afterwards. Easiest way is to tape it to the pcb while soldering switches so the switches are soldered within the switch openings. 
